...
<main>
  <div class="main">
    ...
    <div class="wrap-btn">
      <button type="button" class="btn h56 full" onclick="location.href='{{ route('front.customer.councel.index') }}'">나의 상담 보기</button>
    </div>
    <div class="wrap-login customer horizontal">
      <div class="login-cont">
        <form action="{{ route('front.customer.councel.store') }}" method="POST" class="form-login counsel" id="counselForm">
          @csrf
...
function validateAndSubmit() {
    ...
    if (title === '') {
        ...
        // But since we use Laravel's error handling for server-side, 
        // we'll primarily rely on simpler alerts or server-side for detailed messages 
        // unless we want to inject spans dynamically.
        hasError = true;
    }
    if (content === '') {
