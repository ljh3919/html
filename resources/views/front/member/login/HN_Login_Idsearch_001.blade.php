...
<main>
  <div class="main">
    <div class="breadcrumb">
      ...
      <a href="{{ route('front.findid') ?? '#' }}" class="item">ID 찾기</a>
    </div>
    <div class="wrap-tit">
        ...
        .then(data => {
          if (data.success) {
            var findIdContent =
              ...
              "<p style='margin-top:5px; font-size:16px;'>ID를 발송하였습니다.</p>" +
              "</div>";
              
            commonModal.show("ID 찾기 성공", findIdContent, function(isConfirm) {
               location.href = "{{ route('front.login') }}";
            });
          } else {
            commonModal.show("알림", data.message || "입력하신 정보와 일치하는 아이디를 찾을 수 없습니다.");
          }
        })
        .catch(error => {
