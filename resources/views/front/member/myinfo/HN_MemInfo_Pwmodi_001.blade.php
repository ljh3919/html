...
<main>
  <div class="main">
    <div class="breadcrumb">
      ...
      <a href="{{ route('front.change_password') ?? '#' }}" class="item">비밀번호 변경</a>
    </div>
    <div class="wrap-tit">
      ...
      <div class="login-cont myinfo">
        <form action="" class="form-login" id="changePasswordForm" onsubmit="return validateChangePassword()">
          ...
          <div class="input-group">
            ...
            />
            <span class="error-message error-m" id="currentPwError" style="display:none;">
          ...
          <div class="input-group">
            ...
            />
            <span class="error-message error-m" id="newPwError" style="display:none;">
              <span class="error-icon">!</span>
              비밀번호 형식이 올바르지 않습니다.
            </span>
          </div>
          ...
          <div class="input-group">
            <input
              ...
              placeholder="변경 비밀번호를 입력해주세요."
            />
            <span class="error-message error-m" id="newPwConfirmError" style="display:none;">
              <span class="error-icon">!</span>
              변경 할 비밀번호가 일치 하지 않습니다.
            </span>
          </div>
          <div class="warn">
            * 비밀번호는 8자리 이상 16자리 이하 영문 알파벳, 숫자,
            특수기호의 조합으로 입력해주세요.
          </div>
          <div class="wrap-btn">
...
<script>
  function validateChangePassword() {
    var currentPwInput = document.getElementById('currentPassword');
    var newPwInput = document.getElementById('newPassword');
    var newPwConfirmInput = document.getElementById('newPasswordConfirm');
    
    var currentPw = currentPwInput.value;
    var newPw = newPwInput.value;
    var newPwConfirm = newPwConfirmInput.value;
    ...
    if (!newPw || newPw.length < 8 || newPw.length > 16) {
    ...
    if (newPw !== newPwConfirm) {
      ...
      document.getElementById('newPwConfirmError').style.display = 'block';
      return false;
    }

    fetch("{{ route('front.change_password.post') }}", {
      method: "POST",
      headers: {
