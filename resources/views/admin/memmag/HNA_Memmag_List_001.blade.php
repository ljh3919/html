...
<div class="wrap-table-control">
    ...
    <form action="{{ route('HNA_Memmag_List_001') }}" method="GET">
        ...
        <tr class="member-row" data-id="{{ $member->id }}" style="cursor: pointer;">
            <td>
                <label class="checkbox-item" onclick="event.stopPropagation();">
                    <input type="checkbox" name="ids[]" value="{{ $member->id }}" class="checkbox-input check-item" />
                </label>
            </td>
...
<!-- pager -->
{{ $members->appends(request()->input())->links('admin.partials.pagination') }}
