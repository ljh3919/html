...
<div class="wrap-table-control">
    <div class="wrap-table-control-left">
        <div class="wrap-count">
            총 등록 수:
            <div class="count">{{ number_format($totalCount) }}건</div>
        </div>
    </div>
...
<table class="table board-table">
    <thead>
        <tr>
            ...
            <th scope="col" style="width: 15%">작성자 ID</th>
    ...
    <tbody>
        ...
        <tr class="inquiry-row" data-id="{{ $inquiry->id }}" style="cursor: pointer;">
            <td {!! $hasReply ? 'rowspan="2"' : '' !!}>
                <label class="checkbox-item" onclick="event.stopPropagation();">
                    <input type="checkbox" name="ids[]" value="{{ $inquiry->id }}" class="checkbox-input check-item" />
                </label>
            </td>
            <td {!! $hasReply ? 'rowspan="2"' : '' !!}>{{ $inquiries->total() - ($inquiries->currentPage() - 1) * $inquiries->perPage() - $loop->index }}</td>
            <td class="text-left ellipsis tal">
                {{ $inquiry->title }}
            </td>
            <td class="{{ $inquiry->status == '답변완료' ? 'done' : '' }}">
                {{ $inquiry->status }}
            </td>
            <td class="text-center">{{ $inquiry->username }}</td>
            <td class="text-secondary text-center">{{ $inquiry->created_at->format('Y-m-d') }}</td>
        </tr>
        @if($hasReply)
        <tr class="inquiry-row" data-id="{{ $inquiry->id }}" style="cursor: pointer;">
            <td class="text-left ellipsis tal reply">
                <span>ㄴ [Re] : {{ $inquiry->reply->title }}</span>
            </td>
            <td class="done"></td>
            <td class="text-center text-secondary">관리자</td>
            <td class="text-secondary text-center">{{ $inquiry->reply->created_at->format('Y-m-d') }}</td>
        </tr>
        @endif
        @empty
        <tr>
            <td colspan="6" class="text-center py-5">검색결과가 없습니다.</td>
        </tr>
        @endforelse
    </tbody>
</table>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    btnDelete.addEventListener('click', function() {
        if (confirm("삭제하시면 데이터를 되돌릴 수 없습니다. 정말 삭제하시겠습니까?")) {
            ...
            fetch("{{ route('admin.inquiry.massDestroy') }}", {
                method: 'POST',
                headers: {
