...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">관리자사이트 > 고객센터 > 공지사항 관리 > 목록</h1>
        <a href="{{ route('HNA_Customer_Noticeregi_001') }}" class="btn btn-primary">등록</a>
    </div>
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-6">
            <form action="{{ route('HNA_Customer_Noticelist_001') }}" method="GET" class="form-inline justify-content-end">
                ...
                <table class="table table-bordered table-hover" id="noticeTable">
                    ...
                    <tbody>
                        @forelse($notices as $notice)
                            <tr onclick="location.href='{{ route('HNA_Customer_Noticeview_001', $notice->id) }}'" style="cursor: pointer;">
                                <td class="text-center">{{ $notices->firstItem() + $loop->index }}</td>
                                <td>{{ $notice->title }}</td>
                                <td class="text-center">{{ $notice->author->name }}</td>
                                <td class="text-center">{{ $notice->created_at->format('Y-m-d') }}</td>
