...
<div class="wrap-table-control">
    ...
    <form action="{{ route('HNA_Lettermag_List_001') }}" method="GET">
...
<table class="table board-table">
    <thead>
        <tr>
            ...
            <th scope="col" style="width: calc((100% - 120px) / 4)">ID</th>
            <th scope="col" style="width: 33%">내용</th>
            <th scope="col" style="width: calc((100% - 120px) / 4)">작성자</th>
            <th scope="col" style="width: calc((100% - 120px) / 4)">비밀글</th>
            <th scope="col" style="width: calc((100% - 120px) / 4)">등록일</th>
        </tr>
    </thead>
    <tbody>
        ...
        <tr class="letter-row" data-id="{{ $letter->id }}" style="cursor: pointer;">
            <td>
                <label class="checkbox-item" onclick="event.stopPropagation();">
                    <input type="checkbox" name="ids[]" value="{{ $letter->id }}" class="checkbox-input check-item" />
                </label>
            </td>
            <td>{{ $letters->total() - ($letters->currentPage() - 1) * $letters->perPage() - $loop->index }}</td>
            <td class="text-secondary text-center">{{ $letter->username }}</td>
            <td class="text-left ellipsis">
                {{ $letter->content }}
            </td>
            <td class="text-center">{{ $letter->author_description }}</td>
            <td class="text-secondary text-center">{{ $letter->is_private == 'Y' ? 'Y' : '' }}</td>
            <td class="text-secondary text-center">{{ $letter->created_at->format('Y-m-d') }}</td>
        </tr>
        @empty
        <tr>
...
<!-- pager -->
{{ $letters->appends(request()->input())->links('admin.partials.pagination') }}
