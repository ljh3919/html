...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">사이버 추모관 > 고인 관리 > 수정</h1>
    </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <form action="{{ route('admin.deadmag.update', $dead->id) }}" method="POST" id="modi-form">
                @csrf
                @method('PUT')
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <input type="text" name="name" class="form-control w-25" value="{{ old('name', $dead->name) }}" required>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div id="location-fields-hall" style="display: {{ old('category', $dead->category) == '하늘누리관' ? 'block' : 'none' }};">
                                        <div class="d-flex align-items-center">
                                            <span class="mr-2">관 :</span>
                                            <select name="location_hall" class="form-control mr-3" style="width: 150px;">
                                                <option value="">선택해주세요</option>
                                                <option value="하늘" {{ $dead->location_hall == '하늘' ? 'selected' : '' }}>하늘관</option>
                                                <option value="누리" {{ $dead->location_hall == '누리' ? 'selected' : '' }}>누리관</option>
                                                <option value="무궁화" {{ $dead->location_hall == '무궁화' ? 'selected' : '' }}>무궁화관</option>
                                            </select>
                                            <span class="mr-2">열 :</span>
                                            <select name="location_row_hall" class="form-control mr-3" style="width: 100px;">
                                                @for($i=1; $i<=10; $i++)
                                                    <option value="{{ $i }}" {{ $dead->location_row == $i ? 'selected' : '' }}>{{ $i }}</option>
                                                @endfor
                                            </select>
                                            <span class="mr-2">번호 :</span>
                                            <input type="text" name="location_num_hall" class="form-control" style="width: 100px;" value="{{ $dead->location_num }}">
                                        </div>
                                    </div>

                                    <div id="location-fields-area" style="display: {{ old('category', $dead->category) == '자연장' ? 'block' : 'none' }};">
                                        <div class="d-flex align-items-center">
                                            <span class="mr-2">구역 :</span>
                                            <select name="location_area" class="form-control mr-3" style="width: 150px;">
                                                <option value="">선택해주세요</option>
                                                <option value="A" {{ $dead->location_area == 'A' ? 'selected' : '' }}>A구역</option>
                                                <option value="B" {{ $dead->location_area == 'B' ? 'selected' : '' }}>B구역</option>
                                                <option value="C" {{ $dead->location_area == 'C' ? 'selected' : '' }}>C구역</option>
                                            </select>
                                            <span class="mr-2">열 :</span>
                                            ...
                                            <span class="mr-2">번호 :</span>
                                            <input type="text" name="location_num_area" class="form-control" style="width: 100px;" value="{{ $dead->location_num }}">
                                        </div>
                                    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        @php
                                            $y = $dead->death_date->format('Y');
                                            $m = $dead->death_date->format('m');
                                            $d = $dead->death_date->format('d');
                                        @endphp
                                        ...
                                        <select name="death_month" class="form-control mr-2" style="width: 80px;" required>
                                            @for($i=1; $i<=12; $i++)
                                                <option value="{{ sprintf('%02d', $i) }}" {{ $m == $i ? 'selected' : '' }}>{{ $i }}월</option>
                                            @endfor
                                        </select>
                                        <select name="death_day" class="form-control mr-2" style="width: 80px;" required>
                                            @for($i=1; $i<=31; $i++)
                                                <option value="{{ sprintf('%02d', $i) }}" {{ $d == $i ? 'selected' : '' }}>{{ $i }}일</option>
                                            @endfor
                                        </select>
                ...
                <div class="text-center mt-4">
                    ...
                    <a href="{{ route('HNA_Deadmag_View_001', $dead->id) }}" class="btn btn-secondary px-5 ml-2">취소</a>
                </div>
            </form>
        </div>
    </div>
