        ...
        <tr>
            <td>
                <div class="wrap-post-info">
                    <div class="date-during">{{ $popup->start_at->format('Y-m-d H:i') }} ~ {{ $popup->end_at->format('Y-m-d H:i') }}</div>
                    <div class="date-write">작성일 : {{ $popup->created_at->format('Y-m-d') }}</div>
                </div>
            </td>
        </tr>
        <tr>
            <td>
                <div class="wrap-popup-preview">
                    <div class="wrap-popup" role="dialog" aria-labelledby="popupPreviewTitle">
                        <div class="popup-header">
                            <h3 class="popup-tit" id="popupPreviewTitle">
                                팝업 미리보기
                            </h3>
                        </div>
...
<div class="wrap-board-btn">
    ...
    <div class="wrap-btn-right">
        ...
        <button type="button" class="btn line small" onclick="deletePopup()">
            <span>삭제</span>
        ...
        <button type="button" class="btn primary small" onclick="location.href='{{ route('HNA_Popup_Modi_001', $popup->id) }}'">
            <span>수정</span>
        </button>
    </div>
</div>
