  ...
  <div class="main">
    ...
    <div class="wrap-login customer">
      <div class="login-cont">
        ...
        <div class="wrap-find">
          <button class="btn empty" id="popupJoinBtn" onclick="location.href='{{ route('frontend.join01') ?? '#' }}'">회원가입</button>
          <button class="btn empty" id="popupFindIdBtn" onclick="location.href='{{ route('frontend.findid') ?? '#' }}'">아이디 찾기</button>
          <button class="btn empty" id="popupFindPasswordBtn" onclick="location.href='{{ route('frontend.findpassword') ?? '#' }}'">비밀번호 찾기</button>
        </div>
      </div>
    </div>
  </div>
...
<script>
  function validateLogin() {
    ...
    fetch("{{ route('frontend.login.post') }}", {
      method: "POST",
      headers: {
