    ...
    <form id="joinForm" class="form-box">
      <div class="input-group">
        ...
        <div class="wrap-form-set">
          ...
          />
          <button type="button" class="btn small h44" id="btnIdCheck">
            <span>중복확인</span>
          </button>
        </div>
      ...
      <div class="input-group">
        ...
        <div class="wrap-form-set">
          ...
          />
          <button type="button" class="btn small h44" id="btnRequestSms">
            <span>인증번호 요청</span>
          </button>
        </div>
      ...
      <div class="input-group" id="smsVerificationGroup">
        ...
        <div class="wrap-form-set">
          ...
          <button type="button" class="btn small h44" id="btnVerifySms" disabled>
            <span>인증확인</span>
          </button>
        </div>
...
<script>
  (function() {
    ...
    window.requestSmsCode = function() {
      ...
      if (!phone) {
        commonModal.show("알림", "핸드폰번호를 입력 후 인증번호요청을 해주시기 바랍니다.");
        return;
      }

      // 시뮬레이션: 6자리 랜덤 숫자
      generatedSmsCode = Math.floor(100000 + Math.random() * 900000).toString();
      console.log("Generated SMS Code (Simulation):", generatedSmsCode);

      commonModal.show("알림", "입력하신 " + phone + "로 인증번호를 발송하였습니다.");
    ...
    window.verifySmsCode = function() {
      ...
      if (inputCode === generatedSmsCode && generatedSmsCode !== "") {
        commonModal.show("알림", "인증이 완료되었습니다.");
        isSmsVerified = true;
        clearInterval(timerInterval);
        ...
        commonModal.show("알림", "인증번호가 올바르지 않습니다. 확인 후 다시 입력해주시기 바랍니다.");
      }
    };
    ...
    window.validateForm = function() {
      ...
      if (!nameRegex.test(userName)) {
        commonModal.show("알림", "한글 이름 5자리까지 입력이 가능합니다.");
        return;
      }
      ...
      fetch("{{ route('frontend.register.post') }}", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
